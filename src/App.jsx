...
// ── Storage (Firestore) ───────────────────────────────────────────────────────
const KEYS = { p:"fm4_p", e:"fm4_e", t:"fm4_t", i:"fm4_i", tr:"fm4_tr", pin:"fm4_pin", epin:"fm4_epin" };
...
function EntryGate({ onPass }) {
  ...
  useEffect(() => {
    load(KEYS.epin, null).then(v => setHasPin(!!v));
  }, []);

  const submit = async () => {
    const stored = await load(KEYS.epin, null);
    if (!stored) {
      if (pin.length < 4) return setErr("Min 4 syfers");
      if (pin !== pin2)   return setErr(T.pinMismatch);
      await save(KEYS.epin, pin);
      onPass();
    } else {
...
export default function App() {
  ...
  // PIN modal state for nav switch
  ...
  // ── Entry gate — shown before anything else ──
  if (!entryPassed) return (
    <><style>{CSS}</style><EntryGate onPass={() => setEntryPassed(true)}/></>
  );
